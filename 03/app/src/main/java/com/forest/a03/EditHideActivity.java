...
public class EditHideActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //从布局文件中获取名叫ll_hide 的线性布局
        LinearLayout ll_hide = findViewById(R.id.ll_hide);
        //从布局文件中获取名叫et_phone的密码编辑框
        EditText et_phone = findViewById(R.id.et_phone);
        //从布局文件中获取名叫et_password的密码编辑框
        EditText et_password = findViewById(R.id.et_password);
        //从布局文件中获取名叫et_other 的其他编辑框
        et_other = findViewById(R.id.et_other);
        ll_hide.setOnClickListener(this);
        //给手机号码编辑框添加文本变化监听器
        et_phone.addTextChangedListener(new HideTextWatccher(et_phone));
        //给密码编辑框添加文本变化监听器
        et_password.addTextChangedListener(new HideTextWatccher(et_password));
    }

    @Override
    public void onClick(View v) {
        if (v.getId() == R.id.ll_hide){
            //实际上不止是et_other 的软键盘会关闭，其他编辑框的软键盘也会关闭
            //因为方法内部去获取视图的WindowToken，这个Token在每个页面上面都是唯一的
            ViewUtil.hideOneInputMethod(EditHideActivity.this,et_other);
        }
    }

    //定义一个编辑框监听器，在输入文本达到指定长度时自动隐藏输入法
    private class HideTextWatccher implements TextWatcher{
        private EditText mView;//声明一个编辑框对象
        private int mMaxLength;//声明一个最大长度变量
        private CharSequence mStr;//声明一个文本串


        public HideTextWatccher(EditText v) {
            super();;
            mView = v;
            //通过反射机制获取编辑框的最大长度
