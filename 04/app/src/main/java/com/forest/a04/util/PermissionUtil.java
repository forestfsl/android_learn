...
public class PermissionUtil {
    ...
    public static boolean checkMultiPermission(Activity act,String[] permissions,int reqeustCode){
        boolean result = true;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            ...
            if (check != PackageManager.PERMISSION_GRANTED) {
                // 未开启该权限，则请求系统弹窗，好让用户选择是否立即开启权限
                ActivityCompat.requestPermissions(act, permissions, reqeustCode);
                result = false;
            }
